...
//*********bot
			const sendMess = (hehe, teks) => {
				samu330.sendMessage(hehe, teks, text, {contextInfo: {"forwardingScore": 9999, "isForwarded": true}}, {quoted: fakeProduct
})
			}
...
${bodyM} ${prefix}anime
${bodyM} ${prefix}cmd *(Escribe un texto para crear logo)* 
...
quoted: fvid})              
  break
			...
			case 'public':
				   publik = true
				   samu330.sendMessage(mek.key.remoteJid, `「 *PUBLIC-MODE* 」`, MessageType.text, { quoted:fakeProduct})
			break
			case 'self':
				   publik = false
				   samu330.sendMessage(mek.key.remoteJid, `「 *SELF-MODE* 」`, MessageType.text, { quoted:fakeProduct})
			break
			case 'hidetag':
					members_id = []
			...
			case 'grupo':
				if (!isGroup) return reply(mess.only.group)
				if (args[0] === 'abierto') {
					samu330.sendMessage(from, `*「 GRUPO CERRADO 」*`, MessageType.text, fakeProduct)
		...
		samu330.sendMessage(from, smyperry, document, {                                                                
			mimetype: 'file', filename: `${data.result.filename}`, quoted: samu330Local
  })	
                //
                break
case '>':
...
break
			...
			case '$':
				const cmd = args.join(' ')
				exec(cmd, (err, stdout) => {
					if (err) return samu330.sendMessage(from, `${err}`, text, { quoted: fakeProduct })
					if (stdout) {
						samu330.sendMessage(from, stdout, text, { quoted:fakeProduct})
					}
				})
				break
				  ...
				  bodyM = args[0]
					...
					case 'trigger':
					   ...
					   exec(`ffmpeg -i ${ger} -filter_complex "acrusher=level_in=8:level_out=18:bits=8:mode=log:aa=1" ${ran}`, (err, stderr, stdout) => {
						...
						samu330.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: fakeProduct})
						fs.unlinkSync(ran)
					})
				break
...
//*********
				...
				var ram2 = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
					...
					samu330.sendMessage(from, teks, text, {quoted: fakeProduct, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
break
  ...
  break
			...
			case 'hp':
				...
				buffer = await getBuffer(`https://videfikri.com/api/textmaker/hpotter/?text=${ween}`)
		   		 samu330.sendMessage(from, buffer, image, {quoted: fakeProduct, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
		    break
		    case 'cmd':
...
samu330.sendMessage(from, shan, image, {quoted: fakeProduct, contextInfo: {"forwardingScore": 999, "isForwarded"
: true}})                                                                                                 //                                                                                                        break
...
samu330.sendMessage(from, shan, image, {quoted: fakeProduct, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
//
break
...
samu330.sendMessage(from, data, image, {
  quoted: fakeDoc, caption: `*⚡Sam y Perry🔥*`, contextInfo: {"forwardingScore": 9999, "isForwarded": true}
})
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 samu330.sendMessage(from, hedhe, image, {quoted: fakeProduct, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
} else {
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 samu330.sendMessage(from, satu, image, {quoted: fakeDoc, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
} else {
  reply('etiqueta una imagen!')
}
break
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 samu330.sendMessage(from, hehe, image, {quoted: fakeProduct, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
} else {
}
break
...
try {
  ...
  samu330.sendMessage(from, pint, image, {
	  quoted: fakeDoc, caption: `${beh.title}`, contextInfo: {"forwardingScore": 9999, "isForwarded": true}
  })
...
case 'ss':
 ...
 samu330.sendMessage(from, anu, image, {quoted: fakeProduct})
 break
...
case 'ig':
  ...
  infomp3 = `*Audio*\n◦ *Nombre* : ${sam330.result.full_name}\n◦ *Usuario* : ${sam330.result.username}\n◦ *Descripcion* : ${sam330.result.caption}\n◦ *Likes* : ${sam330.result.like}\n◦ *Comentarios* : ${sam330.result.comment}\n◦ *Duracion* : ${sam330.result.duration}\n_Por si el archivo ne se envia, descarge directamente desde aqui:_\n◦ *Link* : ${sam330.result.video}`		
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ${addMetadata('StMv-By:-Samu-y-Perry')} ${ran} -o ${ran}`, async (error) => {
									if (error) return reply(mess.error.stick)
									samu330.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: samu330Local, contextInfo: {"forwardingScore": 9999, "isForwarded": true}})
                                                                        reply(mess.success)
									fs.unlinkSync(media)
									fs.unlinkSync(ran)
								})
...
reply(mess.wait)
	anu = await fetchJson(`https://api-anoncybfakeplayer.herokuapp.com/pastebin?text=${paste}`, {method: 'get'})
        samu330.sendMessage(from, `${anu.result}`, text, {quoted: fakeProduct, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
                   break
...
//*********
				...
				case 'ttp':
                    ...
                    exec(`wget ${anu} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
                                                fs.unlinkSync(ranp)
                                                if (err) return reply(mess.error.stick)
                                                nobg = fs.readFileSync(rano)
                                                samu330.sendMessage(from, nobg, sticker, {quoted: mek})
...
//clear all chat
				...
				case 'bc':
					...
					anu = await samu330.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : fakeProduct
...
//*********Mengambil link grup
				...
				samu330.sendMessage(from, `https://chat.whatsapp.com/${linkgc}`, text, {quoted:fakeDoc})
				break
			case 'entrabot':
					samu330.query({
