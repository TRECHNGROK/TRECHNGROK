		...
		message: {
			"productMessage": {
				"product": {
					...
					"description": "",
					"currencyCode": "IDR",
					"priceAmount1000": `${samu.harga_toko}`,
					"retailerId": "NyanBot",
...
⍣ *BOT INFO* ⍣
${samu}◦ 🔋 nivel de bateria :${samu} ${global.batrei[global.batrei.length - 1]}%
...
//*********google voice
				...
				if (args.length < 2) return samu330.sendMessage(from, 'Y el texto??', text, { quoted: ftoko })
				dtt = args.join(' ')
				ranm = getRandom('.mp3')
				rano = getRandom('.ogg')
				dtt.length > 300
